...
def _recompute_real_mlir_launch_override(
    *,
    ...
    """
    Re-materialize launch configs for dynamic-shape real-MLIR kernels.

    Some contracts persist a concrete `cuda_real_mlir_launch_override` from the
    shape used when the artifact was first generated. That is correct for
    fixed-shape smoke runs, but it becomes stale when callers intentionally
    relower the same PTX with different symbolic bindings (notably perf runs
    using `--shape ...`).

    Keep this narrowly scoped to kernel kinds whose launch shape we can
    reconstruct exactly from runtime bindings.
    """

    kernel_kind = str((artifacts or {}).get("cuda_real_mlir_kernel_kind") or "").strip()
    if kernel_kind != "batch_norm2d_channelwise_v1":
        return dict(fallback) if isinstance(fallback, Mapping) else None

    c_dim = _try_int((merged_bindings or {}).get("C"))
    if c_dim is None or c_dim <= 0:
        return dict(fallback) if isinstance(fallback, Mapping) else None

    fallback_map = dict(fallback) if isinstance(fallback, Mapping) else {}
    block = fallback_map.get("block") if isinstance(fallback_map.get("block"), list) else []
    shared_mem = _try_int(fallback_map.get("shared_mem"))
    threads = _try_int((artifacts or {}).get("cuda_real_mlir_batch_norm_threads"))
    if threads is None or threads <= 0:
        try:
            threads = int(block[0]) if len(block) >= 1 else None
        except Exception:
            threads = None
    if threads is None or threads <= 0:
        threads = 32
    ...
    return {
        ...
        "shared_mem": int(shared_mem) if shared_mem is not None else 0,
        "origin": "real_mlir_recomputed",
    }
...
def lower_cuda_contract_to_kernel(
    ...
    *,
    ...
    if exe_format in {"cuda_ptx", "ptx"}:
        ...
        explicit_launch_override = _recompute_real_mlir_launch_override(
            artifacts=dict(contract.artifacts or {}),
            merged_bindings=merged_bindings,
            fallback=(contract.artifacts or {}).get("cuda_real_mlir_launch_override"),
        )
