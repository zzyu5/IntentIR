...
static mlir::Value subgroupAllReduceSumF32(mlir::OpBuilder &b, mlir::Location loc, mlir::Value v,
                                           int32_t width) {
  auto widthC = makeI32Const(b, loc, width);
  mlir::Value cur = v;
  for (int32_t offset = width / 2; offset >= 1; offset /= 2) {
    auto off = makeI32Const(b, loc, offset);
    auto sh = b.create<mlir::gpu::ShuffleOp>(loc, cur, off, widthC, mlir::gpu::ShuffleMode::XOR);
    cur = b.create<mlir::arith::AddFOp>(loc, cur, sh.getResult(0)).getResult();
  }
  return cur;
}

static mlir::Value subgroupBroadcastF32(mlir::OpBuilder &b, mlir::Location loc, mlir::Value v,
                                        int32_t lane, int32_t width) {
  auto laneC = makeI32Const(b, loc, lane);
  auto widthC = makeI32Const(b, loc, width);
  return b.create<mlir::gpu::ShuffleOp>(loc, v, laneC, widthC, mlir::gpu::ShuffleMode::IDX)
      .getResult(0);
}

static mlir::LogicalResult lowerCudaLayerNormPersistentAxis1V1(LoweringContext &ctx) {
  if (ctx.outputs.size() != 3) {
    ctx.module.emitError("layer_norm_persistent: expected 3 outputs (out, mean, rstd)");
    return mlir::failure();
  }
  ...
  auto isF32Tensor = [&](llvm::StringRef name) -> bool {
    auto it = ctx.tensors.find(name.str());
    return it != ctx.tensors.end() && llvm::StringRef(it->second.dtype).trim().lower() == "f32";
  };

  std::string outName;
  std::string meanName;
  std::string rstdName;
  if (llvm::is_contained(ctx.outputs, std::string("out_ptr")) &&
      llvm::is_contained(ctx.outputs, std::string("out_mean_ptr")) &&
      llvm::is_contained(ctx.outputs, std::string("out_rstd_ptr"))) {
    outName = "out_ptr";
    meanName = "out_mean_ptr";
    rstdName = "out_rstd_ptr";
  } else {
    for (const auto &nm : ctx.outputs) {
      ...
      if (shOr->size() == 2) {
        ...
        if (meanName.empty())
          meanName = nm;
        else if (rstdName.empty())
          rstdName = nm;
      }
    }
  }
  if (outName.empty() || meanName.empty() || rstdName.empty()) {
    ctx.module.emitError(
        "layer_norm_persistent: failed to identify rank-2 out and rank-1 mean/rstd outputs");
    return mlir::failure();
  }

  auto shapeOutOr = resolveShape(ctx.tensors[outName], ctx.shapeBindings);
  auto shapeMeanOr = resolveShape(ctx.tensors[meanName], ctx.shapeBindings);
  auto shapeRstdOr = resolveShape(ctx.tensors[rstdName], ctx.shapeBindings);
  if (mlir::failed(shapeOutOr) || mlir::failed(shapeMeanOr) || mlir::failed(shapeRstdOr)) {
    ctx.module.emitError("layer_norm_persistent: failed to resolve output shapes");
    return mlir::failure();
  }
  int64_t M = (*shapeOutOr)[0];
  int64_t N = (*shapeOutOr)[1];
  if (shapeMeanOr->size() != 1 || (*shapeMeanOr)[0] != M || shapeRstdOr->size() != 1 ||
      (*shapeRstdOr)[0] != M) {
    ctx.module.emitError("layer_norm_persistent: mean/rstd outputs must be shape [M]");
    return mlir::failure();
  }

  if (N <= 0 || N > 4096) {
    // Keep large-row cases on the full196 correctness-first path for now.
    return mlir::success();
  }

  std::set<std::string> outSet(ctx.outputs.begin(), ctx.outputs.end());
  auto takeIfPresent = [&](llvm::StringRef name) -> std::string {
    if (outSet.count(name.str()) != 0)
      return "";
    auto it = ctx.tensors.find(name.str());
    if (it == ctx.tensors.end())
      return "";
  ...
  std::string inputName = takeIfPresent("in_ptr");
  std::string weightName = takeIfPresent("weight_ptr");
  std::string biasName = takeIfPresent("bias_ptr");

  if (inputName.empty() || weightName.empty() || biasName.empty()) {
    for (const auto &nm : ctx.argOrder) {
      ...
      if (shOr->size() == 1 && (*shOr)[0] == N) {
        if (weightName.empty()) {
          weightName = nm;
        } else if (biasName.empty() && nm != weightName) {
          biasName = nm;
        }
      }
    }
  ...
  if (inputName.empty() || weightName.empty() || biasName.empty()) {
    ctx.module.emitError("layer_norm_persistent: failed to infer input/weight/bias tensors");
    return mlir::failure();
  }
  if (!isF32Tensor(inputName) || !isF32Tensor(weightName) || !isF32Tensor(biasName) ||
      !isF32Tensor(outName) || !isF32Tensor(meanName) || !isF32Tensor(rstdName)) {
    ctx.module.emitError("layer_norm_persistent: expected f32 input/weight/bias/output tensors");
    return mlir::failure();
  }
  ...
  auto shapeBOr = resolveShape(ctx.tensors[biasName], ctx.shapeBindings);
  if (mlir::failed(shapeInOr) || mlir::failed(shapeWOr) || mlir::failed(shapeBOr)) {
    ctx.module.emitError("layer_norm_persistent: failed to resolve input shapes");
    return mlir::failure();
  }
  if (shapeInOr->size() != 2 || (*shapeInOr)[0] != M || (*shapeInOr)[1] != N) {
    ctx.module.emitError("layer_norm_persistent: input tensor must be shape [M, N]");
    return mlir::failure();
  }
  if (shapeWOr->size() != 1 || (*shapeWOr)[0] != N || shapeBOr->size() != 1 ||
      (*shapeBOr)[0] != N) {
    ctx.module.emitError("layer_norm_persistent: weight/bias tensors must be shape [N]");
    return mlir::failure();
  }
  ...
  auto shTy =
      mlir::MemRefType::get({2 * warps}, f32, mlir::MemRefLayoutAttrInterface{}, sharedMemSpace);
  auto shName = "__intentir_sh_layernorm_" + sanitizeSymbolName(ctx.kernelName) + "_f32";
  ...
  auto Mean = getArgByName(ctx, fn, meanName);
  auto Rstd = getArgByName(ctx, fn, rstdName);
  if (!In || !W || !Bias || !Out || !Mean || !Rstd) {
    ctx.module.emitError("layer_norm_persistent: failed to map kernel args");
    return mlir::failure();
  }

  auto in2Ty = mlir::MemRefType::get({M, N}, f32, mlir::MemRefLayoutAttrInterface{}, globalMemSpace);
  auto out2Ty =
      mlir::MemRefType::get({M, N}, f32, mlir::MemRefLayoutAttrInterface{}, globalMemSpace);
  auto In2 =
      mlir::memref::ReinterpretCastOp::create(b, loc, in2Ty, In, 0, {M, N}, {N, 1}).getResult();
  auto Out2 =
      mlir::memref::ReinterpretCastOp::create(b, loc, out2Ty, Out, 0, {M, N}, {N, 1}).getResult();
  ...
  auto cWarps = makeIndexConst(b, loc, warps);
  auto c0f = makeF32Const(b, loc, 0.0f);
  auto nVal = makeF32Const(b, loc, static_cast<float>(N));
  ...
  } else if (auto epsConst = constEps()) {
    epsVal = makeF32Const(b, loc, *epsConst);
  } else {
    ctx.module.emitError("layer_norm_persistent: missing eps scalar (neither arg nor const)");
    return mlir::failure();
  }
  ...
  auto sumFor =
      b.create<mlir::scf::ForOp>(loc, tid, cN, cThreads, mlir::ValueRange{c0f, c0f});
  b.setInsertionPointToStart(sumFor.getBody());
  auto j = sumFor.getInductionVar();
  auto accSum = sumFor.getRegionIterArgs()[0];
  auto accSq = sumFor.getRegionIterArgs()[1];
  auto x = b.create<mlir::memref::LoadOp>(loc, In2, mlir::ValueRange{row, j}).getResult();
  auto sumNext = b.create<mlir::arith::AddFOp>(loc, accSum, x).getResult();
  auto sqNext = b.create<mlir::math::FmaOp>(loc, x, x, accSq).getResult();
  llvm::SmallVector<mlir::Value, 2> yieldVals{sumNext, sqNext};
  b.create<mlir::scf::YieldOp>(loc, yieldVals);
  b.setInsertionPointAfter(sumFor);
  auto partialSum = sumFor.getResult(0);
  auto partialSq = sumFor.getResult(1);

  auto warpSum = warpAllReduceSumF32(b, loc, partialSum);
  auto warpSq = warpAllReduceSumF32(b, loc, partialSq);
  ...
  auto warpSqIdx = b.create<mlir::arith::AddIOp>(loc, warpIdx, cWarps).getResult();
  b.create<mlir::memref::StoreOp>(loc, warpSum, Sh, mlir::ValueRange{warpIdx});
  b.create<mlir::memref::StoreOp>(loc, warpSq, Sh, mlir::ValueRange{warpSqIdx});
  ...
  auto laneIn =
      b.create<mlir::arith::CmpIOp>(loc, mlir::arith::CmpIPredicate::ult, laneI32, makeI32Const(b, loc, warps))
          .getResult();
  auto loadIf = b.create<mlir::scf::IfOp>(loc, mlir::TypeRange{f32, f32}, laneIn, /*withElse=*/true);
  ...
  auto laneSqIdx = b.create<mlir::arith::AddIOp>(loc, laneIdx, cWarps).getResult();
  auto warpSumIn = b.create<mlir::memref::LoadOp>(loc, Sh, mlir::ValueRange{laneIdx}).getResult();
  auto warpSqIn = b.create<mlir::memref::LoadOp>(loc, Sh, mlir::ValueRange{laneSqIdx}).getResult();
  llvm::SmallVector<mlir::Value, 2> warpLoadYield{warpSumIn, warpSqIn};
  b.create<mlir::scf::YieldOp>(loc, warpLoadYield);
  b.setInsertionPointToStart(&loadIf.getElseRegion().front());
  llvm::SmallVector<mlir::Value, 2> warpZeroYield{c0f, c0f};
  b.create<mlir::scf::YieldOp>(loc, warpZeroYield);
  b.setInsertionPointAfter(loadIf);

  auto totalSum = warpAllReduceSumF32(b, loc, loadIf.getResult(0));
  auto totalSq = warpAllReduceSumF32(b, loc, loadIf.getResult(1));
  auto ifWarpLane0 = b.create<mlir::scf::IfOp>(loc, mlir::TypeRange{}, laneIs0, /*withElse=*/false);
  b.setInsertionPointToStart(&ifWarpLane0.getThenRegion().front());
  auto meanV = b.create<mlir::arith::DivFOp>(loc, totalSum, nVal).getResult();
  auto ex2 = b.create<mlir::arith::DivFOp>(loc, totalSq, nVal).getResult();
  auto meanSq = b.create<mlir::arith::MulFOp>(loc, meanV, meanV).getResult();
  auto varRaw = b.create<mlir::arith::SubFOp>(loc, ex2, meanSq).getResult();
  auto varV = b.create<mlir::arith::MaximumFOp>(loc, varRaw, c0f).getResult();
  auto varEps = b.create<mlir::arith::AddFOp>(loc, varV, epsVal).getResult();
  auto rstdV = b.create<mlir::math::RsqrtOp>(loc, varEps).getResult();
  b.create<mlir::memref::StoreOp>(loc, meanV, Mean, mlir::ValueRange{row});
  b.create<mlir::memref::StoreOp>(loc, rstdV, Rstd, mlir::ValueRange{row});
  b.create<mlir::memref::StoreOp>(loc, meanV, Sh, mlir::ValueRange{c0});
  b.create<mlir::memref::StoreOp>(loc, rstdV, Sh, mlir::ValueRange{c1});
  b.setInsertionPointAfter(ifWarpLane0);
  b.setInsertionPointAfter(ifWarp0);
  b.create<mlir::gpu::BarrierOp>(loc);

  auto meanAll = b.create<mlir::memref::LoadOp>(loc, Sh, mlir::ValueRange{c0}).getResult();
  auto rstdAll = b.create<mlir::memref::LoadOp>(loc, Sh, mlir::ValueRange{c1}).getResult();
  ...
  auto bv = b.create<mlir::memref::LoadOp>(loc, Bias, mlir::ValueRange{jj}).getResult();
  auto centered = b.create<mlir::arith::SubFOp>(loc, xv, meanAll).getResult();
  auto normalized = b.create<mlir::arith::MulFOp>(loc, centered, rstdAll).getResult();
  auto scaled = b.create<mlir::arith::MulFOp>(loc, normalized, wv).getResult();
  auto y = b.create<mlir::arith::AddFOp>(loc, scaled, bv).getResult();
  ...
  ctx.module->setAttr("intentir.lowering_kind", mlir::StringAttr::get(mlirCtx, "cuda_focus_v1"));
  ctx.module->setAttr("intentir.cuda_real_mlir_kernel_kind",
                      mlir::StringAttr::get(mlirCtx, "layer_norm_axis1_v1"));
  mergeIntentirMetaJson(ctx.module, [&](llvm::json::Object &meta) {
    ...
    meta["cuda_real_mlir_kernel_kind"] = "layer_norm_axis1_v1";
...
static mlir::LogicalResult lowerCudaBatchNorm2dChannelwiseV1(LoweringContext &ctx) {
  constexpr llvm::StringLiteral inputName = "input";
  constexpr llvm::StringLiteral weightName = "weight";
  constexpr llvm::StringLiteral biasName = "bias";
  constexpr llvm::StringLiteral runningMeanName = "running_mean";
  constexpr llvm::StringLiteral runningVarName = "running_var";
  constexpr llvm::StringLiteral epsName = "eps";
  constexpr llvm::StringLiteral momentumName = "momentum";
  constexpr llvm::StringLiteral nElementsName = "n_elements";
  constexpr llvm::StringLiteral nMinus1Name = "n_minus_1";
  constexpr llvm::StringLiteral outName = "output_1";
  constexpr llvm::StringLiteral meanName = "mean";
  constexpr llvm::StringLiteral invStdName = "inv_std";
  constexpr llvm::StringLiteral runningMeanOutName = "running_mean_out";
  constexpr llvm::StringLiteral runningVarOutName = "running_var_out";

  auto requireTensor = [&](llvm::StringRef name) -> bool {
    if (ctx.tensors.count(name.str()) != 0) {
      return true;
    }
    ctx.module.emitError() << "batch_norm2d: missing tensor spec for " << name;
    return false;
  };
  for (llvm::StringRef name : {inputName, weightName, biasName, runningMeanName, runningVarName,
                               epsName, momentumName, nElementsName, nMinus1Name, outName,
                               meanName, invStdName, runningMeanOutName, runningVarOutName}) {
    if (!requireTensor(name)) {
      return mlir::failure();
    }
  }

  auto expectShape = [&](llvm::StringRef name, size_t rank) -> mlir::FailureOr<std::vector<int64_t>> {
    auto shOr = resolveShape(ctx.tensors[name.str()], ctx.shapeBindings);
    if (mlir::failed(shOr)) {
      ctx.module.emitError() << "batch_norm2d: failed to resolve shape for " << name;
      return mlir::failure();
    }
    if (shOr->size() != rank) {
      ctx.module.emitError() << "batch_norm2d: expected rank " << rank << " for " << name
                             << ", got " << shOr->size();
      return mlir::failure();
    }
    return *shOr;
  ...
  auto expectF32 = [&](llvm::StringRef name) -> bool {
    if (llvm::StringRef(ctx.tensors[name.str()].dtype).trim().lower() == "f32") {
      return true;
    }
    ctx.module.emitError() << "batch_norm2d: expected f32 tensor " << name
                           << ", got " << ctx.tensors[name.str()].dtype;
    return false;
  };
  for (llvm::StringRef name : {inputName, weightName, biasName, runningMeanName, runningVarName,
                               epsName, momentumName, nElementsName, nMinus1Name, outName,
  ...
  auto inShapeOr = expectShape(inputName, 3);
  auto weightShapeOr = expectShape(weightName, 1);
  auto biasShapeOr = expectShape(biasName, 1);
  auto runningMeanShapeOr = expectShape(runningMeanName, 1);
  auto runningVarShapeOr = expectShape(runningVarName, 1);
  auto outShapeOr = expectShape(outName, 3);
  auto meanShapeOr = expectShape(meanName, 1);
  auto invStdShapeOr = expectShape(invStdName, 1);
  auto runningMeanOutShapeOr = expectShape(runningMeanOutName, 1);
  auto runningVarOutShapeOr = expectShape(runningVarOutName, 1);
  if (mlir::failed(inShapeOr) || mlir::failed(weightShapeOr) || mlir::failed(biasShapeOr) ||
      mlir::failed(runningMeanShapeOr) || mlir::failed(runningVarShapeOr) ||
      mlir::failed(outShapeOr) || mlir::failed(meanShapeOr) ||
      mlir::failed(invStdShapeOr) || mlir::failed(runningMeanOutShapeOr) ||
      mlir::failed(runningVarOutShapeOr)) {
    return mlir::failure();
  }

  const int64_t N = (*inShapeOr)[0];
  const int64_t C = (*inShapeOr)[1];
  const int64_t HW = (*inShapeOr)[2];
  if (N <= 0 || C <= 0 || HW <= 0) {
    ctx.module.emitError() << "batch_norm2d: invalid input shape [" << N << "," << C << "," << HW << "]";
    return mlir::failure();
  }
  if (*outShapeOr != *inShapeOr) {
    ctx.module.emitError("batch_norm2d: output_1 shape must match input shape");
    return mlir::failure();
  }
  for (const auto &sh : {*weightShapeOr, *biasShapeOr, *runningMeanShapeOr, *runningVarShapeOr,
                         *meanShapeOr, *invStdShapeOr, *runningMeanOutShapeOr,
                         *runningVarOutShapeOr}) {
    if (sh.size() != 1 || sh[0] != C) {
      ctx.module.emitError("batch_norm2d: channel vectors must all have shape [C]");
      return mlir::failure();
    }
  }

  const int64_t elems = N * HW;
  const int64_t chwStride = C * HW;
  int64_t threads = 32;
  while (threads < elems && threads < 256) {
    threads <<= 1;
  }
  threads = std::min<int64_t>(256, std::max<int64_t>(32, threads));
  const int64_t warps = std::max<int64_t>(1, threads / 32);
  if ((threads % 32) != 0 || warps > 8) {
    ctx.module.emitError() << "batch_norm2d: invalid launch config threads=" << threads
                           << " warps=" << warps;
    return mlir::failure();
  }
  ...
  auto sharedMemSpace = mlir::IntegerAttr::get(mlir::IntegerType::get(mlirCtx, 64), 3);
  mlir::MemRefType shTy;
  std::string shName;
  if (warps > 1) {
    shTy = mlir::MemRefType::get({2 * warps}, f32, mlir::MemRefLayoutAttrInterface{}, sharedMemSpace);
    shName = "__intentir_sh_batchnorm_" + sanitizeSymbolName(ctx.kernelName) + "_f32";
    auto align16 = b.getI64IntegerAttr(16);
    (void)mlir::memref::GlobalOp::create(b, loc, shName, b.getStringAttr("private"), shTy,
                                         /*initial_value=*/{}, /*constant=*/false, align16);
  }
  ...
  auto W = getArgByName(ctx, fn, weightName);
  auto Bias = getArgByName(ctx, fn, biasName);
  auto RunningMean = getArgByName(ctx, fn, runningMeanName);
  auto RunningVar = getArgByName(ctx, fn, runningVarName);
  auto Eps = getArgByName(ctx, fn, epsName);
  auto Momentum = getArgByName(ctx, fn, momentumName);
  auto NElements = getArgByName(ctx, fn, nElementsName);
  auto NMinus1 = getArgByName(ctx, fn, nMinus1Name);
  auto Out = getArgByName(ctx, fn, outName);
  auto Mean = getArgByName(ctx, fn, meanName);
  auto InvStd = getArgByName(ctx, fn, invStdName);
  auto RunningMeanOut = getArgByName(ctx, fn, runningMeanOutName);
  auto RunningVarOut = getArgByName(ctx, fn, runningVarOutName);
  if (!In || !W || !Bias || !RunningMean || !RunningVar || !Eps || !Momentum || !NElements ||
      !NMinus1 || !Out || !Mean || !InvStd || !RunningMeanOut || !RunningVarOut) {
    ctx.module.emitError("batch_norm2d: failed to map kernel args");
    return mlir::failure();
  }

  auto tid = b.create<mlir::gpu::ThreadIdOp>(loc, mlir::gpu::Dimension::x).getResult();
  auto chan = b.create<mlir::gpu::BlockIdOp>(loc, mlir::gpu::Dimension::x).getResult();
  auto tidI32 = b.create<mlir::arith::IndexCastOp>(loc, b.getI32Type(), tid).getResult();
  auto c32i = makeI32Const(b, loc, 32);
  auto laneI32 = b.create<mlir::arith::RemUIOp>(loc, tidI32, c32i).getResult();
  auto warpI32 = b.create<mlir::arith::DivUIOp>(loc, tidI32, c32i).getResult();
  ...
  auto c0 = makeIndexConst(b, loc, 0);
  auto c1 = makeIndexConst(b, loc, 1);
  auto cElems = makeIndexConst(b, loc, elems);
  auto cBatch = makeIndexConst(b, loc, N);
  auto cHW = makeIndexConst(b, loc, HW);
  auto cCHW = makeIndexConst(b, loc, chwStride);
  auto cThreads = makeIndexConst(b, loc, threads);
  auto cWarps = makeIndexConst(b, loc, warps);
  auto c0f = makeF32Const(b, loc, 0.0f);
  auto c1f = makeF32Const(b, loc, 1.0f);
  const bool smallSpatialFast = (warps == 1 && HW <= 8);
  int32_t smallSpatialWidth = 32;
  if (smallSpatialFast) {
    smallSpatialWidth = 1;
    while (smallSpatialWidth < HW) {
      smallSpatialWidth <<= 1;
    }
    smallSpatialWidth = std::min<int32_t>(32, smallSpatialWidth);
  }

  auto epsVal = b.create<mlir::memref::LoadOp>(loc, Eps, mlir::ValueRange{c0}).getResult();
  auto momentumVal = b.create<mlir::memref::LoadOp>(loc, Momentum, mlir::ValueRange{c0}).getResult();
  auto nElementsVal = b.create<mlir::memref::LoadOp>(loc, NElements, mlir::ValueRange{c0}).getResult();
  auto nMinus1Val = b.create<mlir::memref::LoadOp>(loc, NMinus1, mlir::ValueRange{c0}).getResult();
  auto chanBase = b.create<mlir::arith::MulIOp>(loc, chan, cHW).getResult();

  mlir::Value partialSum;
  mlir::Value partialSq;
  mlir::Value smallSpatialLaneActive;
  if (smallSpatialFast) {
    smallSpatialLaneActive =
        b.create<mlir::arith::CmpIOp>(loc, mlir::arith::CmpIPredicate::ult, tid, cHW).getResult();
    auto partialIf =
        b.create<mlir::scf::IfOp>(loc, mlir::TypeRange{f32, f32}, smallSpatialLaneActive, /*withElse=*/true);
    b.setInsertionPointToStart(&partialIf.getThenRegion().front());
    auto laneBase = b.create<mlir::arith::AddIOp>(loc, chanBase, tid).getResult();
    auto fastFor = b.create<mlir::scf::ForOp>(loc, c0, cBatch, c1, mlir::ValueRange{c0f, c0f});
    b.setInsertionPointToStart(fastFor.getBody());
    auto nIdx = fastFor.getInductionVar();
    auto accSum = fastFor.getRegionIterArgs()[0];
    auto accSq = fastFor.getRegionIterArgs()[1];
    auto nBase = b.create<mlir::arith::MulIOp>(loc, nIdx, cCHW).getResult();
    auto xIdx = b.create<mlir::arith::AddIOp>(loc, nBase, laneBase).getResult();
    auto x = b.create<mlir::memref::LoadOp>(loc, In, mlir::ValueRange{xIdx}).getResult();
    auto sumNext = b.create<mlir::arith::AddFOp>(loc, accSum, x).getResult();
    auto sqNext = b.create<mlir::math::FmaOp>(loc, x, x, accSq).getResult();
    llvm::SmallVector<mlir::Value, 2> fastYield{sumNext, sqNext};
    b.create<mlir::scf::YieldOp>(loc, fastYield);
    b.setInsertionPointAfter(fastFor);
    llvm::SmallVector<mlir::Value, 2> partialYield{fastFor.getResult(0), fastFor.getResult(1)};
    b.create<mlir::scf::YieldOp>(loc, partialYield);
    b.setInsertionPointToStart(&partialIf.getElseRegion().front());
    llvm::SmallVector<mlir::Value, 2> zeroYield{c0f, c0f};
    b.create<mlir::scf::YieldOp>(loc, zeroYield);
    b.setInsertionPointAfter(partialIf);
    partialSum = partialIf.getResult(0);
    partialSq = partialIf.getResult(1);
  } else {
    auto sumFor =
        b.create<mlir::scf::ForOp>(loc, tid, cElems, cThreads, mlir::ValueRange{c0f, c0f});
    b.setInsertionPointToStart(sumFor.getBody());
    auto i = sumFor.getInductionVar();
    auto accSum = sumFor.getRegionIterArgs()[0];
    auto accSq = sumFor.getRegionIterArgs()[1];
    auto nIdx = b.create<mlir::arith::DivUIOp>(loc, i, cHW).getResult();
    auto hwIdx = b.create<mlir::arith::RemUIOp>(loc, i, cHW).getResult();
    auto nBase = b.create<mlir::arith::MulIOp>(loc, nIdx, cCHW).getResult();
    auto base = b.create<mlir::arith::AddIOp>(loc, nBase, chanBase).getResult();
    auto xIdx = b.create<mlir::arith::AddIOp>(loc, base, hwIdx).getResult();
    auto x = b.create<mlir::memref::LoadOp>(loc, In, mlir::ValueRange{xIdx}).getResult();
    auto sumNext = b.create<mlir::arith::AddFOp>(loc, accSum, x).getResult();
    auto sqNext = b.create<mlir::math::FmaOp>(loc, x, x, accSq).getResult();
    llvm::SmallVector<mlir::Value, 2> sumYield{sumNext, sqNext};
    b.create<mlir::scf::YieldOp>(loc, sumYield);
    b.setInsertionPointAfter(sumFor);
    partialSum = sumFor.getResult(0);
    partialSq = sumFor.getResult(1);
  }

  auto warpSum = smallSpatialFast ? subgroupAllReduceSumF32(b, loc, partialSum, smallSpatialWidth)
                                  : warpAllReduceSumF32(b, loc, partialSum);
  auto warpSq = smallSpatialFast ? subgroupAllReduceSumF32(b, loc, partialSq, smallSpatialWidth)
                                 : warpAllReduceSumF32(b, loc, partialSq);
  ...
  if (warps == 1) {
    auto meanV = b.create<mlir::arith::DivFOp>(loc, warpSum, nElementsVal).getResult();
    auto ex2 = b.create<mlir::arith::DivFOp>(loc, warpSq, nElementsVal).getResult();
    auto meanSq = b.create<mlir::arith::MulFOp>(loc, meanV, meanV).getResult();
    auto varRaw = b.create<mlir::arith::SubFOp>(loc, ex2, meanSq).getResult();
    auto varV = b.create<mlir::arith::MaximumFOp>(loc, varRaw, c0f).getResult();
    auto varEps = b.create<mlir::arith::AddFOp>(loc, varV, epsVal).getResult();
    auto invV = b.create<mlir::math::RsqrtOp>(loc, varEps).getResult();
    auto threadIs0 =
        b.create<mlir::arith::CmpIOp>(loc, mlir::arith::CmpIPredicate::eq, tid, c0).getResult();
    auto if0 = b.create<mlir::scf::IfOp>(loc, mlir::TypeRange{}, threadIs0, /*withElse=*/false);
    b.setInsertionPointToStart(&if0.getThenRegion().front());
    b.create<mlir::memref::StoreOp>(loc, meanV, Mean, mlir::ValueRange{chan});
    b.create<mlir::memref::StoreOp>(loc, invV, InvStd, mlir::ValueRange{chan});
    auto rmIn = b.create<mlir::memref::LoadOp>(loc, RunningMean, mlir::ValueRange{chan}).getResult();
    auto rvIn = b.create<mlir::memref::LoadOp>(loc, RunningVar, mlir::ValueRange{chan}).getResult();
    auto oneMinus = b.create<mlir::arith::SubFOp>(loc, c1f, momentumVal).getResult();
    auto rmKeep = b.create<mlir::arith::MulFOp>(loc, oneMinus, rmIn).getResult();
    auto rmDelta = b.create<mlir::arith::MulFOp>(loc, momentumVal, meanV).getResult();
    auto rmOut = b.create<mlir::arith::AddFOp>(loc, rmKeep, rmDelta).getResult();
    b.create<mlir::memref::StoreOp>(loc, rmOut, RunningMeanOut, mlir::ValueRange{chan});
    auto bessel = b.create<mlir::arith::DivFOp>(loc, nElementsVal, nMinus1Val).getResult();
    auto unbiased = b.create<mlir::arith::MulFOp>(loc, varV, bessel).getResult();
    auto rvKeep = b.create<mlir::arith::MulFOp>(loc, oneMinus, rvIn).getResult();
    auto rvDelta = b.create<mlir::arith::MulFOp>(loc, momentumVal, unbiased).getResult();
    auto rvOut = b.create<mlir::arith::AddFOp>(loc, rvKeep, rvDelta).getResult();
    b.create<mlir::memref::StoreOp>(loc, rvOut, RunningVarOut, mlir::ValueRange{chan});
    ...
    auto ifWarpLane0 = b.create<mlir::scf::IfOp>(loc, mlir::TypeRange{}, laneIs0, /*withElse=*/false);
    b.setInsertionPointToStart(&ifWarpLane0.getThenRegion().front());
    auto meanV = b.create<mlir::arith::DivFOp>(loc, totalSum, nElementsVal).getResult();
    auto ex2 = b.create<mlir::arith::DivFOp>(loc, totalSq, nElementsVal).getResult();
    auto meanSq = b.create<mlir::arith::MulFOp>(loc, meanV, meanV).getResult();
    auto varRaw = b.create<mlir::arith::SubFOp>(loc, ex2, meanSq).getResult();
    auto varV = b.create<mlir::arith::MaximumFOp>(loc, varRaw, c0f).getResult();
    auto varEps = b.create<mlir::arith::AddFOp>(loc, varV, epsVal).getResult();
    ...
    auto rvOut = b.create<mlir::arith::AddFOp>(loc, rvKeep, rvDelta).getResult();
    b.create<mlir::memref::StoreOp>(loc, rvOut, RunningVarOut, mlir::ValueRange{chan});
    auto c1Sh = makeIndexConst(b, loc, 1);
    auto meanSh = makeIndexConst(b, loc, 0);
    b.create<mlir::memref::StoreOp>(loc, meanV, Sh, mlir::ValueRange{meanSh});
    b.create<mlir::memref::StoreOp>(loc, invV, Sh, mlir::ValueRange{c1Sh});
    b.create<mlir::memref::StoreOp>(loc, varV, Sh, mlir::ValueRange{cWarps});
    b.setInsertionPointAfter(ifWarpLane0);
    b.setInsertionPointAfter(ifWarp0);
    b.create<mlir::gpu::BarrierOp>(loc);
    meanAll = b.create<mlir::memref::LoadOp>(loc, Sh, mlir::ValueRange{c0}).getResult();
    auto c1Load = makeIndexConst(b, loc, 1);
    invAll = b.create<mlir::memref::LoadOp>(loc, Sh, mlir::ValueRange{c1Load}).getResult();
    varAll = b.create<mlir::memref::LoadOp>(loc, Sh, mlir::ValueRange{cWarps}).getResult();
  }

  (void)varAll;
  auto weightVal = b.create<mlir::memref::LoadOp>(loc, W, mlir::ValueRange{chan}).getResult();
  auto biasVal = b.create<mlir::memref::LoadOp>(loc, Bias, mlir::ValueRange{chan}).getResult();
  if (smallSpatialFast) {
    auto outIf = b.create<mlir::scf::IfOp>(loc, mlir::TypeRange{}, smallSpatialLaneActive, /*withElse=*/false);
    b.setInsertionPointToStart(&outIf.getThenRegion().front());
    auto laneBase = b.create<mlir::arith::AddIOp>(loc, chanBase, tid).getResult();
    auto outFastFor = b.create<mlir::scf::ForOp>(loc, c0, cBatch, c1);
    b.setInsertionPointToStart(outFastFor.getBody());
    auto nIdx = outFastFor.getInductionVar();
    auto outNBase = b.create<mlir::arith::MulIOp>(loc, nIdx, cCHW).getResult();
    auto outIdx = b.create<mlir::arith::AddIOp>(loc, outNBase, laneBase).getResult();
    auto ox = b.create<mlir::memref::LoadOp>(loc, In, mlir::ValueRange{outIdx}).getResult();
    auto centered = b.create<mlir::arith::SubFOp>(loc, ox, meanAll).getResult();
    auto normalized = b.create<mlir::arith::MulFOp>(loc, centered, invAll).getResult();
    auto scaled = b.create<mlir::arith::MulFOp>(loc, normalized, weightVal).getResult();
    auto y = b.create<mlir::arith::AddFOp>(loc, scaled, biasVal).getResult();
    b.create<mlir::memref::StoreOp>(loc, y, Out, mlir::ValueRange{outIdx});
    b.setInsertionPointAfter(outFastFor);
    b.setInsertionPointAfter(outIf);
  } else {
    auto outFor = b.create<mlir::scf::ForOp>(loc, tid, cElems, cThreads);
    b.setInsertionPointToStart(outFor.getBody());
    auto oi = outFor.getInductionVar();
    auto on = b.create<mlir::arith::DivUIOp>(loc, oi, cHW).getResult();
    auto ohw = b.create<mlir::arith::RemUIOp>(loc, oi, cHW).getResult();
    auto outNBase = b.create<mlir::arith::MulIOp>(loc, on, cCHW).getResult();
    auto outBase = b.create<mlir::arith::AddIOp>(loc, outNBase, chanBase).getResult();
    auto outIdx = b.create<mlir::arith::AddIOp>(loc, outBase, ohw).getResult();
    auto ox = b.create<mlir::memref::LoadOp>(loc, In, mlir::ValueRange{outIdx}).getResult();
    auto centered = b.create<mlir::arith::SubFOp>(loc, ox, meanAll).getResult();
    auto normalized = b.create<mlir::arith::MulFOp>(loc, centered, invAll).getResult();
    auto scaled = b.create<mlir::arith::MulFOp>(loc, normalized, weightVal).getResult();
  ...
  ctx.module->setAttr("intentir.lowering_kind", mlir::StringAttr::get(mlirCtx, "cuda_focus_v1"));
  ctx.module->setAttr("intentir.cuda_real_mlir_kernel_kind",
                      mlir::StringAttr::get(mlirCtx, "batch_norm2d_channelwise_v1"));
  mergeIntentirMetaJson(ctx.module, [&](llvm::json::Object &meta) {
    meta["schema_version"] = "intentir_meta_v1";
    meta["compiler_stack"] = "cpp_plugin";
    meta["lowering_kind"] = "cuda_focus_v1";
    meta["cuda_real_mlir_kernel_kind"] = "batch_norm2d_channelwise_v1";
    meta["cuda_real_mlir_launch_override"] =
        makeCudaLaunchOverride(/*bx=*/threads, /*by=*/1, /*bz=*/1, /*gx=*/C, /*gy=*/1, /*gz=*/1,
                               /*sharedMem=*/0);
    meta["cuda_real_mlir_output_total"] = static_cast<int64_t>(N * C * HW);
    meta["cuda_real_mlir_batch_norm_threads"] = threads;
  });
  return mlir::success();
}
...
static mlir::LogicalResult lowerCudaFull196GraphV1(LoweringContext &ctx) {
  ...
  int64_t threadsNeeded = std::max<int64_t>(1, maxOutTotal);
  int64_t threads = std::min<int64_t>(256, std::max<int64_t>(32, threadsNeeded));
  if (threads % 32 != 0)
    threads = ((threads + 31) / 32) * 32;
  threads = std::min<int64_t>(256, std::max<int64_t>(32, threads));
  int64_t gridX = std::max<int64_t>(1, (threadsNeeded + threads - 1) / threads);
  mergeIntentirMetaJson(ctx.module, [&](llvm::json::Object &meta) {
    meta["schema_version"] = "intentir_meta_v1";
    meta["compiler_stack"] = "cpp_plugin";
...
public:
  ...
  void runOnOperation() override {
    ...
    } else if (k == "batch_norm2d") {
      if (!kindOverride.empty() && kindOverride != "batch_norm2d_channelwise_v1") {
        module.emitError() << "invalid intentir.kernel_kind_override for batch_norm2d: " << kindOverride
                           << "; allowed=[batch_norm2d_channelwise_v1]";
        ok = mlir::failure();
      } else {
        ok = lowerCudaBatchNorm2dChannelwiseV1(lc);
      }
    } else if (k == "layer_norm_persistent") {
      if (!kindOverride.empty() && kindOverride != "layer_norm_axis1_v1") {
        module.emitError() << "invalid intentir.kernel_kind_override for layer_norm_persistent: "
                           << kindOverride << "; allowed=[layer_norm_axis1_v1]";
        ok = mlir::failure();
      } else {
        ok = lowerCudaLayerNormPersistentAxis1V1(lc);
      }
    } else {
