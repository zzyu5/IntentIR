...
def _infer_compiler_cpp_wave_for_kernel(kernel: str) -> str:
    ...
    for p in state.glob("compiler_cpp_wave*.json"):
        m = re.match(r"compiler_cpp_wave(\d+)_kernels\.json$", p.name)
        if m is None:
            m = re.match(r"compiler_cpp_wave(\d+)_shape_gated_kernels\.json$", p.name)
        if not m:
            continue
        try:
            n = int(m.group(1))
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    suite.add_argument(
        ...
        default="graph",
        help="Benchmark mode forwarded to scripts/flaggems/run_gpu_perf_graph.py. "
        "Use graph for paper-grade pure-compute timing; eager is kept for debugging only.",
    )
    suite.add_argument(
