...
def _release_cuda_bench_state() -> None:
    """
    Best-effort cleanup between kernels to reduce long-run GPU pressure.

    Large graph-captured kernels (for example wide reductions such as `row_sum`)
    can transiently reserve multi-GB CUDA memory pools even when the row/result
    data itself is small. In long multi-kernel sweeps, explicitly releasing
    Python references and cached CUDA blocks between kernels makes the benchmark
    loop less likely to wedge remote GPUs due to cumulative graph/runtime state.
    """
...
def _normalize_kernel_list(values: Any) -> set[str]:
    out: set[str] = set()
...
def _load_gate_policy(path: Path | None) -> tuple[set[str], set[str], dict[str, Any], bool]:
    if path is None:
    ...
    gate_excludes = _normalize_kernel_list(values)
    run_excludes = _normalize_kernel_list(payload.get("run_exclude_kernels"))
    return gate_excludes, run_excludes, payload, True


def _normalize_cuda_sm_label(raw: str | None) -> str:
    text = str(raw or "").strip().lower()
    if not text:
        return ""
    if text.startswith("sm_"):
        return text
    if text.startswith("sm") and len(text) > 2:
        return f"sm_{text[2:]}"
    if text.isdigit():
        return f"sm_{text}"
    return text
...
def _policy_mode_run_excludes(
    ...
    *,
    ...
    mode = str(bench_mode or "").strip().lower()
    sm = _normalize_cuda_sm_label(cuda_sm)
    out: set[str] = set()

    def _merge(value: Any) -> None:
        out.update(_normalize_kernel_list(value))

    by_mode = payload.get("run_exclude_kernels_by_bench_mode")
    if isinstance(by_mode, dict):
        _merge(by_mode.get(mode))
        if mode == "graph_or_eager":
            _merge(by_mode.get("graph"))

    by_sm = payload.get("run_exclude_kernels_by_sm")
    if isinstance(by_sm, dict) and sm:
        _merge(by_sm.get(sm))

    by_mode_sm = payload.get("run_exclude_kernels_by_bench_mode_and_sm")
    if isinstance(by_mode_sm, dict):
        mode_bucket = by_mode_sm.get(mode)
        if isinstance(mode_bucket, dict) and sm:
            _merge(mode_bucket.get(sm))
        if mode == "graph_or_eager":
            graph_bucket = by_mode_sm.get("graph")
            if isinstance(graph_bucket, dict) and sm:
                _merge(graph_bucket.get(sm))
...
def _bench_graph(
    ...
    *,
    ...
    #
    # Important for "pure compute" timing: capture `iters` launches into a single
    # graph replay and measure that replay with CUDA events. This removes the
    # per-iteration Python / launch submission overhead from the steady-state
    # timing window and leaves us with amortized device execution time.
...
def _bench_graph_pair(
    ...
    *,
    ...
    def _safe_stream_sync(*, side: str) -> None:
        try:
            capture_stream.synchronize()
        except Exception as e:  # noqa: BLE001
            raise _BenchFailure(side=side, mode="graph", cause=e) from e
    ...
    # Synchronize after each launch so async CUDA failures are attributed to the
    # offending side instead of surfacing later as an uncaught stream error.
    with torch.cuda.stream(capture_stream):
        _safe_call(native_fn, side="native")
        _safe_stream_sync(side="native")
        _safe_call(intent_fn, side="intentir")
        _safe_stream_sync(side="intentir")
        for _ in range(max(0, int(warmup))):
            _safe_call(native_fn, side="native")
            _safe_stream_sync(side="native")
            _safe_call(intent_fn, side="intentir")
    ...
    try:
        ...
        if isinstance(e, _BenchFailure):
            raise
        raise _BenchFailure(side="intentir", mode="graph", cause=e) from e
...
def _bench_eager_pair(
    ...
    *,
    ...
    torch.cuda.synchronize()
    _safe_call(native_fn, side="native")
    _safe_sync(side="native")
    _safe_call(intent_fn, side="intentir")
    _safe_sync(side="intentir")
    for _ in range(max(0, int(warmup))):
        _safe_call(native_fn, side="native")
        _safe_sync(side="native")
        _safe_call(intent_fn, side="intentir")
...
def _classify_native_launch_kind(*, spec_source: str, launch_source: str) -> str:
    spec = str(spec_source or "").strip()
    launch = str(launch_source or "").strip()
    if "grid_launch" in launch:
        return "direct_grid_launch"
    if launch.startswith("kernel_adapter:"):
        return "kernel_adapter"
    if launch.startswith("torch."):
        return "torch_api_wrapper"
    if launch.startswith("flag_gems_ops."):
        return "flaggems_op_wrapper"
    if launch == "heuristic_signature":
        if spec == "triton_native":
            return "triton_callable"
        if spec == "flaggems_native":
            return "flaggems_callable"
...
def _bench_kernel(
    *,
    ...
    row: dict[str, Any] = {
        ...
        "native_launch_error": "",
        "timing_scope": "",
        "launch_overhead_suppressed": False,
    ...
    intent_fn: Callable[[], None] | None = None
    if not bool(native_only):
        try:
    ...
    try:
        ...
        row["native_launch_kind"] = _classify_native_launch_kind(
            spec_source=str(row.get("native_spec_source") or ""),
    ...
    if bool(native_only):
        ...
        try:
            ...
            row["reason_code"] = _reason_code_from_exception(e, native=True)
            row["reason_detail"] = f"{type(e).__name__}: {e}"
            row["skip_reason"] = ("native_eager_failed" if not graph_enabled else "native_graph_failed")
            row["native_launch_error"] = f"{type(e).__name__}: {e}"
            return row

        row["timing_scope"] = ("cuda_graph_batched_replay" if graph_enabled else "eager_direct_calls")
        row["launch_overhead_suppressed"] = bool(graph_enabled)
        row.update(
            {
                ...
                "capture_ms_intentir": 0.0,
                "replay_ms_intentir": 0.0,
                "replay_ms_total_intentir": 0.0,
                "ratio": None,
                "reason_code": "native_only",
                "reason_detail": f"native-only benchmark completed (bench_mode={row['bench_mode']})",
                "skip_reason": "native_only",
                "count_in_denominator": False,
                "ok": True,
            }
        )
...
def _safe_artifact_token(raw: str) -> str:
...
def _bench_kernel_in_subprocess(
    *,
    kernel: str,
    family: str,
    chunk_name: str,
    ...
    worker_out = (
        Path(out_root)
        / "_isolated"
        / _safe_artifact_token(str(family))
        / _safe_artifact_token(str(kernel))
    )
    worker_out.mkdir(parents=True, exist_ok=True)
    worker_log = worker_out / "worker_stdout.log"
    worker_graph = worker_out / "gpu_perf_graph.json"
    ...
    cmd = [
        ...
        str(kernel),
        "--bench-mode",
        str(cli_args.bench_mode),
        "--warmup",
        str(int(cli_args.warmup)),
        "--iters",
        str(int(cli_args.iters)),
        "--repeats",
        str(int(cli_args.repeats)),
        "--family-kernel-chunk-size",
        "1",
        "--threshold",
        str(float(cli_args.threshold)),
        "--p50-threshold",
        str(float(cli_args.p50_threshold)),
        "--progress-style",
        "none",
        "--no-isolate-kernel-process",
        "--no-stream",
        "--no-resume",
        "--monitor-only",
        "--cuda-runtime-backend",
        str(cli_args.cuda_runtime_backend),
        "--intent-artifact-dir",
        str(cli_args.intent_artifact_dir),
    ]
    if bool(getattr(cli_args, "native_only", False)):
        cmd.append("--native-only")
    if getattr(cli_args, "coverage_batches", None) is not None:
        cmd.extend(["--coverage-batches", str(cli_args.coverage_batches)])
    if getattr(cli_args, "policy_json", None) is not None:
        cmd.extend(["--policy-json", str(cli_args.policy_json)])
    if getattr(cli_args, "tuning_db", None) is not None:
        cmd.extend(["--tuning-db", str(cli_args.tuning_db)])
    for key, value in sorted(dict(shape_overrides or {}).items()):
        cmd.extend(["--shape", f"{str(key)}={int(value)}"])

    timeout_s = float(worker_timeout_sec or 0.0)
    ...
    timed_out = False
    try:
        stdout_text, _ = proc.communicate(timeout=(timeout_s if timeout_s > 0.0 else None))
        proc_stdout = str(stdout_text or "")
    except subprocess.TimeoutExpired as e:
        timed_out = True
        proc_stdout = str((e.stdout or "") if isinstance(e.stdout, str) else (e.stdout or b"").decode("utf-8", errors="replace"))
        with contextlib.suppress(Exception):
            os.killpg(proc.pid, signal.SIGKILL)
        with contextlib.suppress(Exception):
            proc.kill()
        with contextlib.suppress(Exception):
            stdout_tail, _ = proc.communicate(timeout=5.0)
            proc_stdout = proc_stdout + str(stdout_tail or "")
    worker_log.write_text(str(proc_stdout or ""), encoding="utf-8")
    if timed_out:
        tail = "\n".join(str(proc_stdout or "").splitlines()[-20:]).strip()
        return {
            "kernel": str(kernel),
            "semantic_op": str(kernel),
            "family": str(family),
            "chunk": str(chunk_name),
            "gpu_name": (torch.cuda.get_device_name(0) if torch.cuda.is_available() else "unavailable"),
            "bench_mode": str(cli_args.bench_mode),
            "dtype": "mixed",
            "shape": {},
            "qps_native": None,
            "qps_intentir": None,
            ...
            "reason_detail": (
                f"isolated worker timed out after {timeout_s:.1f}s"
            ...
            "intentir_tuning_source": "none",
            "intentir_tuning_applied": {},
            "isolated_worker_out": _to_repo_rel(worker_out),
            "isolated_worker_log": _to_repo_rel(worker_log),
            "isolated_worker_rc": int(proc.returncode if proc.returncode is not None else -9),
            "isolated_worker_timeout_sec": float(timeout_s),
        }

    if worker_graph.is_file():
        try:
            payload = _load_json(worker_graph)
            entries = [e for e in list(payload.get("entries") or []) if isinstance(e, dict)]
            for entry in entries:
                if str(entry.get("kernel") or "").strip() == str(kernel).strip():
                    row = dict(entry)
                    row["chunk"] = str(chunk_name)
                    row["isolated_worker_out"] = _to_repo_rel(worker_out)
                    row["isolated_worker_log"] = _to_repo_rel(worker_log)
                    row["isolated_worker_rc"] = int(proc.returncode if proc.returncode is not None else 0)
                    if timeout_s > 0.0:
                        row["isolated_worker_timeout_sec"] = float(timeout_s)
                    return row
        except Exception:
            pass
    ...
    return {
        ...
        "reason_detail": (
            f"isolated worker failed rc={int(proc.returncode)}"
            + (f"; tail={tail}" if tail else "")
        ),
        ...
        "isolated_worker_out": _to_repo_rel(worker_out),
        "isolated_worker_log": _to_repo_rel(worker_log),
        "isolated_worker_rc": int(proc.returncode if proc.returncode is not None else -1),
        "isolated_worker_timeout_sec": float(timeout_s) if timeout_s > 0.0 else 0.0,
    }
...
def _skipped_row_policy_run_excluded(
    *,
    kernel: str,
    family: str,
    chunk_name: str,
    ...
    row: dict[str, Any] = {
        ...
        "replay_ms_native": 0.0,
        "replay_ms_intentir": 0.0,
        "reason_code": "policy_run_excluded",
        "reason_detail": "excluded by perf policy (known slow or unstable kernel)",
        "skip_reason": "policy_run_excluded",
        "count_in_denominator": False,
        "ok": False,
        "execution_engine": "",
        "contract_schema_version": "",
...
def _to_status_entries(rows: list[dict[str, Any]], *, threshold: float) -> list[dict[str, Any]]:
    ...
    for row in list(rows):
        ...
        native_only = bool(row.get("native_only"))
        ratio = row.get("ratio")
        ...
        if native_only and isinstance(row.get("qps_native"), (int, float)):
            status = "native_only"
        elif (not counted) and (
            reason in {"allowlist_excluded", "policy_run_excluded"}
            or skip_reason in {"allowlist_excluded", "policy_run_excluded"}
        ):
            status = "skipped"
        else:
...
def _aggregate_device(rows: list[dict[str, Any]], *, threshold: float) -> list[dict[str, Any]]:
    ...
    for dev, items in sorted(by_dev.items(), key=lambda kv: kv[0]):
        ...
        native_only_rows = [
            r for r in items if bool(r.get("native_only")) and isinstance(r.get("qps_native"), (int, float))
        ]
        if native_only_rows and not measured:
            passed = list(native_only_rows)
            failed = []
            min_ratio = None
            kernel_measured = int(len(native_only_rows))
        else:
        ...
        out.append(
            {
                ...
                "kernel_measured": int(kernel_measured),
                "kernel_pass": int(len(passed)),
                "kernel_fail": int(len(failed)),
                "ok": bool(kernel_measured > 0 and len(failed) == 0),
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        default="graph",
        help="Benchmark mode. graph captures `iters` invocations into one CUDA Graph replay and times replay with CUDA events "
        "(minimizes Python/launch overhead); eager uses CUDA events around direct calls; "
        "graph_or_eager falls back to eager when graph capture fails.",
    )
    ap.add_argument(
        "--native-only",
        action="store_true",
        help="Benchmark only the native Triton/FlagGems baseline without requiring IntentIR lowering/runtime.",
    )
    ap.add_argument(
        "--isolate-kernel-process",
        action=argparse.BooleanOptionalAction,
        default=None,
        help="Run each benchmarked kernel in a fresh subprocess. "
        "Defaults to auto for broad graph-only sweeps; pass --no-isolate-kernel-process to disable.",
    )
    ap.add_argument(
        "--kernel-worker-timeout-sec",
        type=float,
        default=0.0,
        help="Optional timeout for isolated per-kernel worker subprocesses. "
        "0 disables the timeout; broad graph sweeps auto-enable a safety timeout.",
    )
    ...
    policy_excluded_kernels, policy_run_excluded_kernels_base, policy_payload, policy_loaded = _load_gate_policy(
        Path(args.policy_json) if args.policy_json is not None else None
    )
    policy_mode_run_excluded_kernels = _policy_mode_run_excludes(
        policy_payload,
        bench_mode=str(args.bench_mode),
        cuda_sm=str(cuda_sm_after),
    )
    policy_run_excluded_kernels = set(policy_run_excluded_kernels_base)
    policy_run_excluded_kernels.update(policy_mode_run_excluded_kernels)
    ...
    if policy_run_excluded_kernels:
        print(
            f"[gpu-perf] run exclude kernels ({len(policy_run_excluded_kernels)}): "
            + ",".join(sorted(policy_run_excluded_kernels)),
            flush=True,
        )
    ...
    auto_isolate_kernel_process = bool(
        str(args.bench_mode).strip().lower() == "graph" and int(kernel_expected_count) > 1
    )
    if args.isolate_kernel_process is None:
        isolate_kernel_process = bool(auto_isolate_kernel_process)
        isolate_mode = "auto"
    else:
        isolate_kernel_process = bool(args.isolate_kernel_process)
        isolate_mode = "forced" if isolate_kernel_process else "disabled"
    if isolate_kernel_process:
        print(
            f"[gpu-perf] kernel subprocess isolation {isolate_mode} "
            f"(bench_mode={args.bench_mode}, kernel_expected_count={kernel_expected_count})",
            flush=True,
        )
    elif args.isolate_kernel_process is False and auto_isolate_kernel_process:
        print(
            f"[gpu-perf] kernel subprocess isolation disabled "
            f"(bench_mode={args.bench_mode}, kernel_expected_count={kernel_expected_count})",
            flush=True,
        )
    kernel_worker_timeout_sec = float(getattr(args, "kernel_worker_timeout_sec", 0.0) or 0.0)
    auto_kernel_worker_timeout_sec = (
        600.0
        if (
            kernel_worker_timeout_sec <= 0.0
            and isolate_kernel_process
            and str(args.bench_mode).strip().lower() == "graph"
            and int(kernel_expected_count) > 1
        )
        else 0.0
    )
    if kernel_worker_timeout_sec <= 0.0 and auto_kernel_worker_timeout_sec > 0.0:
        kernel_worker_timeout_sec = float(auto_kernel_worker_timeout_sec)
    if isolate_kernel_process and kernel_worker_timeout_sec > 0.0:
        timeout_mode = "auto" if auto_kernel_worker_timeout_sec > 0.0 else "explicit"
        print(
            f"[gpu-perf] isolated worker timeout {timeout_mode} "
            f"({kernel_worker_timeout_sec:.1f}s per kernel)",
            flush=True,
        )
        ...
        for cidx, ckernels in chunk_iter:
            ...
            if not bool(args.dry_run):
                for kernel in ckernels:
                    try:
                        if str(kernel) in policy_run_excluded_kernels:
                            row = _skipped_row_policy_run_excluded(
                                kernel=str(kernel),
                                family=family,
                        ...
                        elif allowlist_enabled and str(kernel) not in allowlist_kernels:
                            ...
                            if isolate_kernel_process:
                                row = _bench_kernel_in_subprocess(
                                    kernel=str(kernel),
                                    family=family,
                                    chunk_name=chunk_name,
                                    cli_args=args,
                                    ...
                                    out_root=out_root,
                                    worker_timeout_sec=float(kernel_worker_timeout_sec),
                                    shape_overrides=(dict(kernel_shape_overrides) if kernel_shape_overrides else None),
                                )
                            else:
                                row = _bench_kernel(
                                    kernel=str(kernel),
    ...
    measured_rows = [r for r in all_rows if bool(r.get("count_in_denominator"))]
    native_only_rows = [r for r in all_rows if bool(r.get("native_only")) and isinstance(r.get("qps_native"), (int, float))]
    excluded_rows = [r for r in all_rows if bool(r.get("gate_excluded"))]
    run_excluded_rows = [r for r in all_rows if bool(r.get("policy_run_excluded"))]
    ...
    invocation_meta = {
        ...
        "policy_run_exclude_kernels": sorted(policy_run_excluded_kernels),
        "kernel_worker_timeout_sec": float(kernel_worker_timeout_sec),
    ...
    p50_threshold = float(args.p50_threshold)
    native_only_mode = bool(args.native_only)
    p50_ok = bool(
        native_only_mode
        or (p50_threshold <= 0.0)
        or (perf_p50_ratio is not None and float(perf_p50_ratio) >= p50_threshold)
    )
    aggregate_ok = bool(
        categories_completed == categories_expected
        and len(categories_failed) == 0
        and ((len(native_only_rows) > 0) if native_only_mode else (len(measured_rows) > 0))
        and device_ok
        and p50_ok
    )
    ...
    gpu_perf_payload = {
        ...
        "native_only": bool(native_only_mode),
        "timing_scope": (
            "cuda_graph_batched_replay"
            if str(bench_mode_label) == "graph"
            else ("mixed_graph_or_eager" if str(bench_mode_label) == "graph_or_eager" else "eager_direct_calls")
        ),
        "shape_overrides": dict(shape_overrides),
        "policy_kernel_shape_overrides": dict(policy_kernel_shapes),
        "intent_artifact_dir": _to_repo_rel(Path(str(args.intent_artifact_dir))),
        ...
        "kernel_allowlist_excluded": int(allowlist_excluded_count),
        "kernel_expected_count": int(kernel_expected_count),
        "kernel_measured_count": int(len(native_only_rows) if native_only_mode else len(measured_rows)),
        ...
        "gate_policy": {
            ...
            "run_exclude_kernels_policy": sorted(policy_run_excluded_kernels),
            "run_exclude_kernels_policy_base": sorted(policy_run_excluded_kernels_base),
            "run_exclude_kernels_policy_mode": sorted(policy_mode_run_excluded_kernels),
            "excluded_rows": int(len(excluded_rows)),
            "run_excluded_rows": int(len(run_excluded_rows)),
    ...
    run_summary = {
        ...
        "contract_schema_version": "intent_mlir_backend_contract_v2",
        "monitor_only": bool(args.monitor_only),
        "gpu_perf_native_only": bool(native_only_mode),
        "coverage_mode": str(coverage_mode),
        ...
        "gpu_perf_kernel_measured": int(len(native_only_rows) if native_only_mode else len(measured_rows)),
        ...
        "gpu_perf_kernel_run_excluded": int(len(run_excluded_rows)),
