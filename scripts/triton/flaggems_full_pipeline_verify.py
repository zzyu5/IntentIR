...
def _clone_spec_with_shapes(spec, overrides: dict[str, int]):
    merged = {str(k): int(v) for k, v in dict(spec.canonical_shapes or {}).items()}
    for key, value in dict(overrides or {}).items():
        merged[str(key)] = int(value)
    if spec.normalize_shapes:
        merged = {
            str(k): int(v)
            for k, v in dict(spec.normalize_shapes(dict(merged)) or {}).items()
        }
    return replace(spec, canonical_shapes=merged)
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        action="append",
        default=[],
        help="Override canonical shape binding(s) for artifact generation, e.g. --shape N=32. "
        "Only supported with exactly one --kernel.",
    )
    ...
    if shape_overrides:
        if len(selected_specs) != 1:
            raise SystemExit("--shape requires exactly one selected kernel; please pass a single --kernel")
        selected_specs = [_clone_spec_with_shapes(selected_specs[0], shape_overrides)]
        print(
            f"[shape-override] kernel={selected_specs[0].name} canonical_shapes={selected_specs[0].canonical_shapes}",
            flush=True,
        )
