...
def _resolve_report_path(raw: object, *, artifact_root: Path) -> Path | None:
    ...
    if p.is_absolute():
        ...
        for anchor in ("artifacts", "workflow", "pipeline", "archive"):
            indexes = [idx for idx, part in enumerate(parts) if part == anchor]
            for idx in reversed(indexes):
                tail = parts[idx:]
                if not tail:
                    continue
                candidates.append((current_root / Path(*tail)).resolve())
        repo_name = str(current_root.name).strip()
        if repo_name:
            indexes = [idx for idx, part in enumerate(parts) if part == repo_name]
            for idx in reversed(indexes):
                tail = parts[idx + 1 :]
                if not tail:
                    continue
...
def _baseline_matches_tensor_specs(
    *,
    ...
    """
    Check whether cached baseline arrays agree with the report-declared shapes.

    This guards against stale `.baseline.npz` files that may survive from older
    experiments with smaller smoke-test shapes. Perf / strict runtime paths must
    not trust those files when the report's `baseline.shapes` now targets a
    different concrete kernel shape.
    """

    if not baseline or not tensor_specs:
        return True

    checked_any = False
    for name, spec in dict(tensor_specs or {}).items():
        if str(name) not in baseline or not isinstance(spec, dict):
            continue
        shape_spec = _tensor_shape_spec(spec)
        expected_shape = _resolve_tensor_shape(shape_spec, bindings)
        if expected_shape is None:
            continue
        actual_shape = tuple(int(v) for v in np.asarray(baseline[str(name)]).shape)
        checked_any = True
        if tuple(expected_shape) != tuple(actual_shape):
            return False
    return True if checked_any else True
...
def _prepare_kernel_context(
    ...
    *,
    ...
    report_bindings = _coerce_bindings(raw_bindings)
    if baseline_npz_path.exists():
        loaded_baseline = dict(np.load(baseline_npz_path, allow_pickle=False))
        if _baseline_matches_tensor_specs(
            ...
            baseline_source = "stale_npz_ignored"
    if (not baseline) and str(frontend) == "triton" and str(triton_provider) in {"native", "flaggems"}:
        # Large baseline snapshots can be intentionally skipped to cap artifact
        # size. Re-materialize provider reference IO on demand so real GPU smoke
        # can still execute and diff against a trustworthy baseline.
        ...
        if str(triton_provider) == "flaggems":
            ...
            backend_target = str(report.get("backend_target") or "cuda_h100")
            flaggems_opset = str(report.get("flaggems_opset") or "deterministic_forward")
            for spec in list(
                default_flaggems_kernel_specs(
                    flaggems_opset=flaggems_opset,
                ...
                spec_map[str(spec.name)] = spec
        else:
            from pipeline.triton.core import coverage_kernel_specs, default_kernel_specs  # noqa: PLC0415

            for spec in list(default_kernel_specs()) + list(coverage_kernel_specs()):
                spec_map[str(spec.name)] = spec
        ...
        baseline_source = f"rerun_triton_{str(triton_provider)}"
