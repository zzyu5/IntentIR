...
def build_current_status_payload(
    *,
    ...
    compiler_cpp_migration: Mapping[str, Any] | None = None,
    compiler_cpp_perf_qualification: Mapping[str, Any] | None = None,
    compiler_cpp_perf_candidates: Mapping[str, Any] | None = None,
    compiler_cpp_perf_backlog: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    cpp_migration = dict(compiler_cpp_migration or {})
    cpp_perf_qualification = dict(compiler_cpp_perf_qualification or {})
    cpp_perf_candidates = dict(compiler_cpp_perf_candidates or {})
    cpp_perf_backlog = dict(compiler_cpp_perf_backlog or {})
    return {
        ...
        "compiler_cpp_migration": {
            "report_path": str(cpp_migration.get("report_path") or ""),
            "selected_wave": str(cpp_migration.get("selected_wave") or ""),
            "available_waves": [str(x) for x in list(cpp_migration.get("available_waves") or []) if str(x).strip()],
            "coverage_kernels_total": int(cpp_migration.get("coverage_kernels_total") or 0),
            "coverage_kernels_with_selected_wave_admission": int(
                cpp_migration.get("coverage_kernels_with_selected_wave_admission") or 0
            ),
            "coverage_kernels_shape_policy_only": int(cpp_migration.get("coverage_kernels_shape_policy_only") or 0),
            "coverage_kernels_not_admitted": int(cpp_migration.get("coverage_kernels_not_admitted") or 0),
            "kernels_total": int(cpp_migration.get("kernels_total") or 0),
            "kernels_with_selected_wave_admission": int(
                cpp_migration.get("kernels_with_selected_wave_admission") or 0
            ),
            "kernels_admitted": int(cpp_migration.get("kernels_admitted") or 0),
            "kernels_shape_gated": int(cpp_migration.get("kernels_shape_gated") or 0),
            "kernels_shape_policy_only": int(cpp_migration.get("kernels_shape_policy_only") or 0),
            "kernels_not_admitted": int(cpp_migration.get("kernels_not_admitted") or 0),
            "shape_gated_kernels": [
                str(x) for x in list(cpp_migration.get("shape_gated_kernels") or []) if str(x).strip()
            ],
            "shape_policy_only_kernels": [
                str(x) for x in list(cpp_migration.get("shape_policy_only_kernels") or []) if str(x).strip()
            ],
            "not_admitted_kernels": [
                str(x) for x in list(cpp_migration.get("not_admitted_kernels") or []) if str(x).strip()
            ],
        },
        "compiler_cpp_perf_qualification": {
            "report_path": str(cpp_perf_qualification.get("report_path") or ""),
            "selected_wave": str(cpp_perf_qualification.get("selected_wave") or ""),
            "ratio_threshold": float(cpp_perf_qualification.get("ratio_threshold") or 0.9),
            "coverage_kernels_total": int(cpp_perf_qualification.get("coverage_kernels_total") or 0),
            "coverage_kernels_with_selected_wave_admission": int(
                cpp_perf_qualification.get("coverage_kernels_with_selected_wave_admission") or 0
            ),
            "coverage_kernels_with_perf_observations": int(
                cpp_perf_qualification.get("coverage_kernels_with_perf_observations") or 0
            ),
            "coverage_kernels_with_any_perf_qualification": int(
                cpp_perf_qualification.get("coverage_kernels_with_any_perf_qualification") or 0
            ),
            "coverage_kernels_perf_qualified": int(
                cpp_perf_qualification.get("coverage_kernels_perf_qualified") or 0
            ),
            "coverage_kernels_kernel_kind_perf_qualified": int(
                cpp_perf_qualification.get("coverage_kernels_kernel_kind_perf_qualified") or 0
            ),
            "coverage_kernels_shape_perf_qualified": int(
                cpp_perf_qualification.get("coverage_kernels_shape_perf_qualified") or 0
            ),
            "coverage_kernels_evidence_below_target": int(
                cpp_perf_qualification.get("coverage_kernels_evidence_below_target") or 0
            ),
            "coverage_kernels_admitted_pending_perf": int(
                cpp_perf_qualification.get("coverage_kernels_admitted_pending_perf") or 0
            ),
            "kernels_total": int(cpp_perf_qualification.get("kernels_total") or 0),
            "kernels_with_selected_wave_admission": int(
                cpp_perf_qualification.get("kernels_with_selected_wave_admission") or 0
            ),
            "kernels_with_perf_observations": int(cpp_perf_qualification.get("kernels_with_perf_observations") or 0),
            "kernels_with_any_perf_qualification": int(
                cpp_perf_qualification.get("kernels_with_any_perf_qualification") or 0
            ),
            "kernels_perf_qualified": int(cpp_perf_qualification.get("kernels_perf_qualified") or 0),
            "kernels_kernel_kind_perf_qualified": int(
                cpp_perf_qualification.get("kernels_kernel_kind_perf_qualified") or 0
            ),
            "kernels_shape_perf_qualified": int(
                cpp_perf_qualification.get("kernels_shape_perf_qualified") or 0
            ),
            "kernels_evidence_below_target": int(
                cpp_perf_qualification.get("kernels_evidence_below_target") or 0
            ),
            "kernels_admitted_pending_perf": int(
                cpp_perf_qualification.get("kernels_admitted_pending_perf") or 0
            ),
            "qualified_kernels": [
                str(x) for x in list(cpp_perf_qualification.get("qualified_kernels") or []) if str(x).strip()
            ],
            "kernel_kind_perf_qualified_kernels": [
                str(x)
                for x in list(cpp_perf_qualification.get("kernel_kind_perf_qualified_kernels") or [])
                if str(x).strip()
            ],
            "shape_perf_qualified_kernels": [
                str(x)
                for x in list(cpp_perf_qualification.get("shape_perf_qualified_kernels") or [])
                if str(x).strip()
            ],
            "evidence_below_target_kernels": [
                str(x)
                for x in list(cpp_perf_qualification.get("evidence_below_target_kernels") or [])
                if str(x).strip()
            ],
            "qualified_cuda_sms": [
                str(x) for x in list(cpp_perf_qualification.get("qualified_cuda_sms") or []) if str(x).strip()
            ],
        },
        "compiler_cpp_perf_candidates": {
            "report_path": str(cpp_perf_candidates.get("report_path") or ""),
            "selected_wave": str(cpp_perf_candidates.get("selected_wave") or ""),
            "ratio_threshold": float(cpp_perf_candidates.get("ratio_threshold") or 0.9),
            "scanned_artifacts_total": int(cpp_perf_candidates.get("scanned_artifacts_total") or 0),
            "scanned_entries_total": int(cpp_perf_candidates.get("scanned_entries_total") or 0),
            "eligible_entries_total": int(cpp_perf_candidates.get("eligible_entries_total") or 0),
            "coverage_kernels_total": int(cpp_perf_candidates.get("coverage_kernels_total") or 0),
            "coverage_kernels_pending_perf_qualification": int(
                cpp_perf_candidates.get("coverage_kernels_pending_perf_qualification") or 0
            ),
            "coverage_candidate_kernels_total": int(
                cpp_perf_candidates.get("coverage_candidate_kernels_total") or 0
            ),
            "coverage_pending_kernels_without_candidates": int(
                cpp_perf_candidates.get("coverage_pending_kernels_without_candidates") or 0
            ),
            "kernels_total": int(cpp_perf_candidates.get("kernels_total") or 0),
            "kernels_pending_perf_qualification": int(
                cpp_perf_candidates.get("kernels_pending_perf_qualification") or 0
            ),
            "candidate_kernels_total": int(cpp_perf_candidates.get("candidate_kernels_total") or 0),
            "pending_kernels_without_candidates": int(
                cpp_perf_candidates.get("pending_kernels_without_candidates") or 0
            ),
            "high_priority_candidates_total": int(cpp_perf_candidates.get("high_priority_candidates_total") or 0),
            "medium_priority_candidates_total": int(cpp_perf_candidates.get("medium_priority_candidates_total") or 0),
            "watch_candidates_total": int(cpp_perf_candidates.get("watch_candidates_total") or 0),
            "low_priority_candidates_total": int(cpp_perf_candidates.get("low_priority_candidates_total") or 0),
            "top_candidate_kernels": [
                str(x) for x in list(cpp_perf_candidates.get("top_candidate_kernels") or []) if str(x).strip()
            ],
            "high_priority_candidate_kernels": [
                str(x)
                for x in list(cpp_perf_candidates.get("high_priority_candidate_kernels") or [])
                if str(x).strip()
            ],
            "medium_priority_candidate_kernels": [
                str(x)
                for x in list(cpp_perf_candidates.get("medium_priority_candidate_kernels") or [])
                if str(x).strip()
            ],
            "watch_candidate_kernels": [
                str(x) for x in list(cpp_perf_candidates.get("watch_candidate_kernels") or []) if str(x).strip()
            ],
            "low_priority_candidate_kernels": [
                str(x) for x in list(cpp_perf_candidates.get("low_priority_candidate_kernels") or []) if str(x).strip()
            ],
            "candidate_cuda_sms": [
                str(x) for x in list(cpp_perf_candidates.get("candidate_cuda_sms") or []) if str(x).strip()
            ],
        },
        "compiler_cpp_perf_backlog": {
            "report_path": str(cpp_perf_backlog.get("report_path") or ""),
            "selected_wave": str(cpp_perf_backlog.get("selected_wave") or ""),
            "ratio_threshold": float(cpp_perf_backlog.get("ratio_threshold") or 0.9),
            "pending_kernels_total": int(cpp_perf_backlog.get("pending_kernels_total") or 0),
            "coverage_pending_kernels_total": int(cpp_perf_backlog.get("coverage_pending_kernels_total") or 0),
            "pending_with_candidate_total": int(cpp_perf_backlog.get("pending_with_candidate_total") or 0),
            "pending_without_candidate_total": int(cpp_perf_backlog.get("pending_without_candidate_total") or 0),
            "coverage_pending_with_candidate_total": int(
                cpp_perf_backlog.get("coverage_pending_with_candidate_total") or 0
            ),
            "coverage_pending_without_candidate_total": int(
                cpp_perf_backlog.get("coverage_pending_without_candidate_total") or 0
            ),
            "formalize_qualification_total": int(cpp_perf_backlog.get("formalize_qualification_total") or 0),
            "archive_then_formalize_qualification_total": int(
                cpp_perf_backlog.get("archive_then_formalize_qualification_total") or 0
            ),
            "collect_focused_kernel_perf_total": int(
                cpp_perf_backlog.get("collect_focused_kernel_perf_total") or 0
            ),
            "archive_then_collect_focused_kernel_perf_total": int(
                cpp_perf_backlog.get("archive_then_collect_focused_kernel_perf_total") or 0
            ),
            "optimize_to_target_total": int(cpp_perf_backlog.get("optimize_to_target_total") or 0),
            "collect_shape_scoped_perf_total": int(
                cpp_perf_backlog.get("collect_shape_scoped_perf_total") or 0
            ),
            "collect_initial_perf_total": int(cpp_perf_backlog.get("collect_initial_perf_total") or 0),
            "top_backlog_kernels": [
                str(x) for x in list(cpp_perf_backlog.get("top_backlog_kernels") or []) if str(x).strip()
            ],
            "coverage_top_backlog_kernels": [
                str(x) for x in list(cpp_perf_backlog.get("coverage_top_backlog_kernels") or []) if str(x).strip()
            ],
            "formalize_qualification_kernels": [
                str(x)
                for x in list(cpp_perf_backlog.get("formalize_qualification_kernels") or [])
                if str(x).strip()
            ],
            "archive_then_formalize_qualification_kernels": [
                str(x)
                for x in list(cpp_perf_backlog.get("archive_then_formalize_qualification_kernels") or [])
                if str(x).strip()
            ],
            "collect_focused_kernel_perf_kernels": [
                str(x)
                for x in list(cpp_perf_backlog.get("collect_focused_kernel_perf_kernels") or [])
                if str(x).strip()
            ],
            "archive_then_collect_focused_kernel_perf_kernels": [
                str(x)
                for x in list(cpp_perf_backlog.get("archive_then_collect_focused_kernel_perf_kernels") or [])
                if str(x).strip()
            ],
            "optimize_to_target_kernels": [
                str(x) for x in list(cpp_perf_backlog.get("optimize_to_target_kernels") or []) if str(x).strip()
            ],
            "collect_shape_scoped_perf_kernels": [
                str(x)
                for x in list(cpp_perf_backlog.get("collect_shape_scoped_perf_kernels") or [])
                if str(x).strip()
            ],
            "collect_initial_perf_kernels": [
                str(x) for x in list(cpp_perf_backlog.get("collect_initial_perf_kernels") or []) if str(x).strip()
            ],
        },
        "script_governance": {
